...
object Part1 extends App {
  ...
  println(s"Total minutes of all the guards $mapGuardTotalMinutes")
  println(s"Total time by guard ${maxTimeGuard._1} ${maxTimeGuard._2}")
  println(s"Total frequencies for the guard ${mapFrequenciesMaxGuard.foreach(x => println(s"${x._1} -> ${x._2}"))}")
  println(s"Most common minute $mostCommonMinute")
  val idGuard: Int = maxTimeGuard._1.substring(1).toInt
  val minuteGuard: Int = mostCommonMinute.toInt * idGuard
  println(s"$idGuard Solution $minuteGuard")
